...
function PasswordField({ name, placeholder, autoComplete }: { name: string; placeholder: string; autoComplete: string }) {
  ...
  return <div className="password-field"><input required name={name} autoComplete={autoComplete} type={visible ? "text" : "password"} placeholder={placeholder} /><button type="button" className="password-toggle" onClick={() => setVisible((current) => !current)} aria-label={visible ? text("Hide password", "Ocultar contraseña") : text("Show password", "Mostrar contraseña")}>{visible ? "👁️" : "🙈"}</button></div>;
}

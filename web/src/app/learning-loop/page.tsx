...
export default function LearningLoopPage() {
  ...
  const outcomeFormRef = useRef<HTMLFormElement | null>(null);
  ...
  const sentProspects = prospects.filter((prospect) => ["sent", "replied", "converted", "lost"].includes(prospect.status));
  const positiveReplies = outcomes.filter((item) => item.kind === "replied_positive").length;
  const meetings = outcomes.filter((item) => item.kind === "meeting").length;
  const orders = outcomes.filter((item) => item.kind === "order").length;
  ...
  return <AppShell><div className="page-toolbar"><SectionHeading eyebrow={text("Learning loop / outcomes", "Ciclo de aprendizaje / resultados")} title={text("Turn real outcomes into a sharper radar.", "Convierte resultados reales en un radar más preciso.")} detail={text("This page starts after real outreach is sent. It never invents activity from unanswered messages.", "Esta página empieza después de que se haya enviado un contacto real. Nunca inventa actividad a partir de mensajes sin respuesta.")} /></div>{notice && <p className="notice" aria-live="polite">{notice}</p>}{error && <p className="error" aria-live="polite">{error}</p>}<div className="learning-layout"><section className="panel learning-hero"><p className="eyebrow">{text("Outcome model", "Modelo de resultados")}</p><h2>{text("The radar learns only", "El radar aprende solo")}<br/><em>{text("from recorded truth.", "de hechos registrados.")}</em></h2><p>{text("Each outcome is tied to a real prospect in the active company. Replies, meetings, orders and losses become feedback for future scoring.", "Cada resultado se vincula a un prospecto real de la empresa activa. Respuestas, reuniones, pedidos y pérdidas sirven de feedback para futuras puntuaciones.")}</p></section><section className="panel learning-progress"><div className="panel-title"><h2>{text("Recorded outcomes", "Resultados registrados")}</h2><span className="feature-status feature-status--ready">Supabase</span></div><div className="learning-stat"><strong>{outcomes.length}</strong><span>{text("total outcomes for this company", "resultados totales de esta empresa")}</span></div><div className="learning-stat"><strong>{positiveReplies}</strong><span>{text("positive replies", "respuestas positivas")}</span></div><div className="learning-stat"><strong>{meetings}</strong><span>{text("meetings", "reuniones")}</span></div><div className="learning-stat"><strong>{orders}</strong><span>{text("orders", "pedidos")}</span></div></section></div><section className="outcome-grid"><div className="section-mini"><p className="eyebrow">{text("What gets recorded", "Qué se registra")}</p><h2>{text("Three layers of truth.", "Tres niveles de verdad.")}</h2><p>{text("These are the operator chores after outreach—not sample results.", "Estas son las tareas del operador después del contacto, no resultados de muestra.")}</p></div>{definitions.map(([en, es, kinds], index) => <article className="outcome-card" key={en}><span className="outcome-number">0{index + 1}</span><strong>{language === "es" ? es : en}</strong><small>{kinds}</small></article>)}</section><section className="panel settings-card outcome-form-panel"><div className="panel-title"><h2>{text("Record a real outcome", "Registrar un resultado real")}</h2><span className="feature-status">{sentProspects.length ? text("Ready", "Listo") : text("Waiting for sent outreach", "Esperando envíos")}</span></div>{sentProspects.length ? <form ref={outcomeFormRef} className="settings-fields" onSubmit={submit}><label>{text("Prospect", "Prospecto")}<select required name="prospect_id" defaultValue=""><option value="" disabled>{text("Select a prospect", "Selecciona un prospecto")}</option>{sentProspects.map((prospect) => <option key={prospect.id} value={prospect.id}>{prospect.name} · {prospect.status}</option>)}</select></label><label>{text("Outcome type", "Tipo de resultado")}<select required name="kind" defaultValue=""><option value="" disabled>{text("Choose one", "Elige uno")}</option><option value="replied_positive">{text("Positive reply", "Respuesta positiva")}</option><option value="replied_negative">{text("Negative reply", "Respuesta negativa")}</option><option value="no_reply">{text("No reply", "Sin respuesta")}</option><option value="meeting">{text("Meeting", "Reunión")}</option><option value="order">{text("Order", "Pedido")}</option><option value="lost">{text("Lost", "Perdido")}</option></select></label><label>{text("Note", "Nota")}<input name="note" placeholder={text("What happened?", "¿Qué ocurrió?")} /></label><button className="button button-primary" disabled={saving}>{saving ? text("Saving…", "Guardando…") : text("Record outcome", "Registrar resultado")}<span className="button-arrow"/></button></form> : <div className="empty-state empty-state--action"><strong>{text("No sent outreach yet.", "Todavía no hay envíos reales.")}</strong><p>{text("First mark a message as sent in Outreach. After that, the prospect appears here for reply, meeting or order tracking.", "Primero marca un mensaje como enviado en Contacto. Después, el prospecto aparecerá aquí para registrar respuesta, reunión o pedido.")}</p><Link href="/enviar">{text("Open outreach", "Abrir contacto")} ↗</Link></div>}</section><section className="panel outcome-records"><div className="panel-title"><h2>{text("Outcome history", "Historial de resultados")}</h2><span>{outcomes.length}</span></div>{loading ? <div className="empty-state">{text("Loading outcomes…", "Cargando resultados…")}</div> : outcomes.length ? outcomes.map((outcome) => <div className="outcome-row" key={outcome.id}><div><strong>{outcome.prospects?.name ?? text("Unknown prospect", "Prospecto desconocido")}</strong><small>{new Date(outcome.created_at).toLocaleDateString(language === "es" ? "es-EC" : "en-US")}</small></div><span className="status-chip">{outcome.kind}</span><p>{outcome.note ?? "—"}</p></div>) : <div className="empty-state empty-state--action"><strong>{text("No outcomes recorded yet.", "Aún no hay resultados registrados.")}</strong><p>{text("Once real outreach receives a response, its result will appear here.", "Cuando un contacto real reciba respuesta, su resultado aparecerá aquí.")}</p><Link href="/enviar">{text("Open outreach", "Abrir contacto")} ↗</Link></div>}</section></AppShell>;
}

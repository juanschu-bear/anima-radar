...
import { deriveBusinessIcp } from "@/lib/business-dna";
...
export async function GET() {
  ...
  const { data, error } = await admin.from("business_profiles").select("id,raw_answers,icp,version,created_at").eq("tenant_id", auth.profile.tenant_id).order("created_at", { ascending: false }).limit(1).maybeSingle();
...
export async function POST(request: Request) {
  ...
  const icp = deriveBusinessIcp(raw, payload.market_lang ?? "en");
  const query = existing
    ? admin.from("business_profiles").update({ raw_answers: raw, icp, version: existing.version + 1 }).eq("id", existing.id)
    : admin.from("business_profiles").insert({ tenant_id: auth.profile.tenant_id, raw_answers: raw, icp });
  const { data, error } = await query.select("id,version,created_at,icp").single();
  if (error) return NextResponse.json({ detail: error.message }, { status: 502 });

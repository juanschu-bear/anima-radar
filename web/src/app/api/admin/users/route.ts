...
export async function POST(request: Request) {
  ...
  for (let attempt = 0; attempt < 25; attempt += 1) {
    ...
    const { error: profileError } = await admin.from("users").insert({ id: created.user.id, tenant_id: actor.tenant_id, email: login, full_name: fullName, role: "member", must_change_password: true });
